...
## PROCEDURE:
STEP 1: Install necessary libraries (opencv-python, numpy).
STEP 2: Use cv2.dnn.readNet to load YOLOv4 model with weights and config.
STEP 4: Use cv2.VideoCapture(0) to start capturing video from the webcam.
STEP 5:Capture frames from the webcam.
STEP 6: Extract bounding boxes, confidence scores, and class IDs from detections.
STEP 7:Display the processed frame in a window.
STEP 8: Exit the loop and close the webcam window when 'q' is pressed.

## PROGRAM:
```
/*
DEEVELOPED BY: HYCINTH D
